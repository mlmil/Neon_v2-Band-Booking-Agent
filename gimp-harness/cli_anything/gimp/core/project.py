...
def create_venue_template(
    ...
    """Create a venue template project with text layers for venue/date/time."""
    canvas_w = alt_width if use_alt else width
    canvas_h = alt_height if use_alt else height
    ...
    layer_mod.add_layer(project, name=venue, layer_type="text", position=len(project["layers"]))
    project["layers"][-1]["text"] = venue
    project["layers"][-1]["font_size"] = 72
    project["layers"][-1]["color"] = "#000000"

    layer_mod.add_layer(project, name=date_text, layer_type="text", position=len(project["layers"]))
    project["layers"][-1]["text"] = date_text
    project["layers"][-1]["font_size"] = 64
    project["layers"][-1]["color"] = "#000000"

    layer_mod.add_layer(project, name=time_text, layer_type="text", position=len(project["layers"]))
    project["layers"][-1]["text"] = time_text
    project["layers"][-1]["font_size"] = 64
    project["layers"][-1]["color"] = "#000000"
