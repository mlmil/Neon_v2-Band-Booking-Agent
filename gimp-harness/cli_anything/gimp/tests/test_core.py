...
class TestLayers:
    ...
    def test_venue_template(self):
        proj = create_venue_template("Ventura Pier", "SAT 5-10", "8-11")
        assert proj["canvas"]["width"] == 2048
        assert proj["canvas"]["height"] == 2048
        assert len(proj["layers"]) == 3
        assert proj["layers"][0]["text"] == "Ventura Pier"
        assert proj["layers"][1]["text"] == "SAT 5-10"
        assert proj["layers"][2]["text"] == "8-11"
