...
@project.command("venue-template")
@click.option("--venue", required=True, help="Venue name")
@click.option("--date", "date_text", required=True, help="Date line text, e.g. SAT 5-10")
@click.option("--time", "time_text", required=True, help="Time line text, e.g. 8-11")
@click.option("--alt-size", is_flag=True, help="Use 1200x600 instead of 2048x2048")
@click.option("--output", "-o", type=str, default=None, help="Save path")
@handle_error
def project_venue_template(venue, date_text, time_text, alt_size, output):
    """Create a venue template project."""
    proj = proj_mod.create_venue_template(
        venue=venue,
        date_text=date_text,
        time_text=time_text,
        use_alt=alt_size,
    ...
    if output:
        proj_mod.save_project(proj, output)
    globals()["output"](proj_mod.get_project_info(proj), f"Created venue template: {venue}")
